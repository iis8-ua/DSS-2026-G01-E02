            ...
            <div class="card shadow-sm border-0">
                <div class="card-header bg-white border-bottom-0 pt-4 pb-0 text-center">
                    <h1 class="h4 mb-0" style="color: #003366;">
                        ...
                        <span class="fs-6 fw-normal text-muted">
                            ({{ \Carbon\Carbon::parse($reserva->hora_inicio)->format('d/m/Y') }})
                        </span>
                    </h1>
                </div>
                <div class="card-body p-4">
                    ...
                    <div class="alert alert-secondary d-flex align-items-center mb-4 border-0 shadow-sm">
                        ...
                        <div>
                            <h6 class="mb-0 fw-bold">Solicitante: {{ $reserva->alumno->getFullName() ?? $reserva->alumno->name ?? 'Desconocido' }}</h6>
                            <small class="mb-0">
                                Tipo de reserva: 
                                <strong>{{ $reserva->reservaGrupal ? 'Grupal (Aforo: ' . $reserva->reservaGrupal->aforo_max . ')' : 'Individual' }}</strong>
                            </small>
                        </div>
                    </div>
                    ...
                    <form action="{{ route('reservas.update', $reserva->id) }}" method="POST">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-4">
                                ...
                                @php
                                    ...
                                    $estadoActual = $reserva->estado->value ?? $reserva->estado;
                                @endphp
                                <select name="estado" class="form-select fw-bold @error('estado') is-invalid @enderror" 
                                    style="color: {{ match($estadoActual) { 'ACEPTADA', 'APROBADA' => '#198754', 'CANCELADA', 'RECHAZADA' => '#dc3545', 'PENDIENTE' => '#fd7e14', default => '#6c757d' } }};">
                                    <option value="PENDIENTE" {{ old('estado', $estadoActual) == 'PENDIENTE' ? 'selected' : '' }} class="text-warning">Pendiente</option>
                                    <option value="ACEPTADA" {{ old('estado', $estadoActual) == 'ACEPTADA' ? 'selected' : '' }} class="text-success">Aceptada / Aprobada</option>
                                    <option value="CANCELADA" {{ old('estado', $estadoActual) == 'CANCELADA' ? 'selected' : '' }} class="text-danger">Cancelada / Rechazada</option>
                                </select>
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="datetime-local" name="hora_inicio" 
                                       value="{{ old('hora_inicio', \Carbon\Carbon::parse($reserva->hora_inicio)->format('Y-m-d\TH:i')) }}" 
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="datetime-local" name="hora_fin" 
                                       value="{{ old('hora_fin', \Carbon\Carbon::parse($reserva->hora_fin)->format('Y-m-d\TH:i')) }}" 
                                       class="form-control @error('hora_fin') is-invalid @enderror">
                                @error('hora_fin')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>
                        </div>

                        {{-- Actualizar reserva o cancelar edición --}}
                        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                            ...
                            <button type="submit" class="btn btn-primary" style="background-color: #003366; border-color: #003366;">
                                <i class="bi bi-save me-1"></i> Actualizar Reserva
                            </button>
                        </div>
